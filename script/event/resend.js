...
module.exports.handleEvent = async function ({ api, event}) {
  ...
  if (event.type == "message_unsend" && msgData.hasOwnProperty(event.messageID)) {
    ...
    if (msgData[event.messageID].attachments.length === 0) {
      api.sendMessage(`${name} قام بحذف هذه الرسالة: ${msgData[event.messageID].body}`, event.threadID);
      ...
      api.sendMessage({ body: `${name} قام بحذف هذه الصورة: ${msgData[event.messageID].body}`, attachment: photo}, event.threadID, () => {
      ...
      api.sendMessage({ body: `${name} قام بحذف هذه الرسالة الصوتية: ${msgData[event.messageID].body}`, attachment: fs.createReadStream('./script/cache/audio.mp3')}, event.threadID, () => {
      ...
      api.sendMessage({ body: `${name} قام بحذف هذه الصورة المتحركة: ${msgData[event.messageID].body}`, attachment: fs.createReadStream('./script/cache/animated_image.gif')}, event.threadID, () => {
