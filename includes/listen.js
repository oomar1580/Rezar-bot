...
models,
  ...
  const checkttDataPath = __dirname + '/../script/checktt/';
  setInterval(async() => {
...
(async function () {
	...
	//DEFINE DATLICH PATH
	const datlichPath = __dirname + '/../script/cache/datlich.json';
		...
		for (el of temp) {
			...
			if ("ATTACHMENT" in el) {
				...
				for (a of el.ATTACHMENT) {
					...
					fs.writeFileSync(__dirname + `/../script/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
					out.attachment.push(fs.createReadStream(__dirname + `/../script/cache/${a.fileName}`));
				}
			}
			console.log(out);
